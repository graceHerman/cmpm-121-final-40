  ...
  private saveGameState(): void
{
  ...
  console.log("State being pushed onto undo stack:", this.cloneState(this.fields));  // Log the state being pushed
  console.log("Push on undo stack:", this.undoStack);  // Log the full undo stack after push
  ...
  private undo() {
    ...
    if (this.undoStack.length > 0) {
      ...
      if (previousState) {
        ...
        this.redoStack.push(this.cloneState(this.fields));
        
        // Restore previous state
        this.restoreState(previousState);
        
        // Log the restored previous state
        console.log("Undo: Restored previous state", previousState);
        //console.log("Undo: Restored previous state", JSON.stringify(previousState, null, 2));

      }
    } else {
      console.log("No more undo actions");
    }
  }
...
private redo() {
  ...
  if (this.redoStack.length > 0) {
    ...
    if (nextState) {
      this.undoStack.push(this.cloneState(this.fields)); // Save current state to undo stack
      this.restoreState(nextState); // Restore the next state
      console.log("Redo: Restored next state", nextState);
    }
  } else {
...
//private cloneState(state: Field[]): Field[] {
  ...
  // Restore the game state from the cloned state
  private restoreState(state: Field[]): void {
    this.fields.forEach((field, index) => {
      ...
      if (savedField) {
        ...
        field.sunLevel = savedField.sunLevel;
        field.plantLevel = savedField.plantLevel;
        //field.sprite.setTexture(savedField.sprite.texture.key); // Restore the sprite texture

        if (savedField.sprite.texture.key) {
          field.sprite.setTexture(savedField.sprite.texture.key);
          console.log("texture: " + savedField.sprite.texture.key);
        }
        else {
          field.sprite.setTexture('field');
          console.log("texture: field");
        }
      }
    });
  }
